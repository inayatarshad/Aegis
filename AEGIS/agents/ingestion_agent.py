...
def ingestion_agent(state: AEGISState) -> AEGISState:
    ...
    try:
        telemetry = DroneTelemetry(
            ...
            iff_signal=bool(raw.get("iff_signal", False)),
            estimated_wingspan_m=float(raw.get("estimated_wingspan_m", 1.0)),
            loiter_detected=bool(raw.get("loiter_detected", False)),
            rapid_altitude_change=bool(raw.get("rapid_altitude_change", False)),
